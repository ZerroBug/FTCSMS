...
try {
    ...
    $stmt->execute([
        $staff_id, $first_name, $surname, $other_names, $dob, $gender,
        $staff_type, $staff_type==='Teaching' ? $email : null, $phone,
        $nationality, $religion, $address, $qualification, $employment_date,
    ...
    if ($staff_type === 'Teaching') {
        ...
        try {
            ...
            $pdo->prepare("
                INSERT INTO email_queue (recipient_email, recipient_name, subject, body, status, error_message)
                VALUES (?, ?, ?, ?, 'pending', ?)
            ")->execute([$email, $first_name.' '.$surname, 'Teaching Staff Login Details', $email_body, $e->getMessage()]);
