    ...
    foreach ($items as $item) {
        ...
        $paidStmt = $pdo->prepare("
            ...
            WHERE student_id = ?
              ...
              AND fee_item_id = ?
        ");
        $paidStmt->execute([$student_id, $academic_year_id, $item['id']]);
        $paid = (float) $paidStmt->fetchColumn();

        $total = (float) $item['amount'];
        $outstanding = max(0, $total - $paid);

        if ($outstanding <= 0) continue; // skip only fully paid items

        /* ===== DETERMINE IF GOODS ===== */
        $isGoods = (strtolower($cat['category_type']) === 'goods');
        ...
        <tr>
            ...
            <td class="text-end">
                '.$input.'
                <input type="hidden" name="fee_category_id['.$item['id'].']" value="'.$cat['id'].'">
                <input type="hidden" name="fee_item_id['.$item['id'].']" value="'.$item['id'].'">
                <input type="hidden" name="outstanding['.$item['id'].']" value="'.$outstanding.'">
            </td>
        </tr>';
    }
}
