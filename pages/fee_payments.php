...
// Students
$students = $pdo->query("
    SELECT s.id, CONCAT(s.first_name,' ',s.surname) AS name,
           c.id AS class_id, c.class_name, c.year_group
    FROM students s
...
?>
                    ...
                    <div class="row g-3">
                        <div class="col-md-6 col-12">
                            ...
                            <select id="student_id" name="student_id" class="form-select" required>
                                ...
                                <option value="<?= $s['id'] ?>" data-class="<?= htmlspecialchars($s['class_name']) ?>"
                                    data-class-id="<?= $s['class_id'] ?>"
                                    data-year-group="<?= htmlspecialchars($s['year_group']) ?>">
                        ...
                        <div class="col-md-4 col-12">
                            ...
                            <input type="hidden" id="year_group">
                        </div>
                        ...
                        <table class="table table-bordered align-middle">
                            <thead class="table-light">
                                <tr>
                                    ...
                                    <th class="text-end">Pay Now / Quantity</th>
                                </tr>
                            </thead>
                            <tbody id="billTable">
                                <tr>
                                    <td colspan="8" class="text-center text-muted">Select student & academic year</td>
                                </tr>
                            </tbody>
                        </table>
                        <p class="note">* For Goods category fees, enter quantity (max 5) instead of amount.</p>
                    </div>
                </div>
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-4 col-12">
                            ...
                            <select name="bank_name" class="form-control" required>
                                ...
                                <option value="ADB">Agricultural Development Bank (ADB)</option>
                                <option value="CalBank">CalBank</option>
                                <option value="Sekyere Rural Bank">Sekyere Rural Bank</option>
                                <option value="Okomfo Anokye Rural Bank">Okomfo Anokye Rural Bank</option>
                                <option value="CBG">Consolidated Bank (CBG)</option>
                                ...
                                <option value="OmniBSIC">OmniBSIC</option>
                                <option value="Prudential">Prudential</option>
                                <option value="Republic Bank">Republic Bank</option>
                                <option value="SocGen">Société Générale (SocGen)</option>
    ...
    // Load Class and Year Group
    $('#student_id').on('change', function() {
        const opt = $(this).find(':selected');
        $('#class_name').val(opt.data('class') || '');
        $('#class_id').val(opt.data('class-id') || '');
        $('#year_group').val(opt.data('year-group') || '');
        loadBills();
    });
    $('#academic_year_id').on('change', loadBills);
    ...
    function loadBills() {
        ...
        if (!student || !year) {
            $('#billTable').html(
                '<tr><td colspan="8" class="text-center text-muted">Select student & academic year</td></tr>');
